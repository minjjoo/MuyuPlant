...
const borderColors = ["#b8d4ed", "#7ab8e8", "#3d7ab5", "#2d5a8e", "#1e3a5f"];

function GoalCircle({ index, label, isMobile }: { index: number; label: string; isMobile: boolean }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: "50%",
      background: "white",
      border: `5px solid ${borderColors[index % borderColors.length]}`,
      display: "flex",
      ...
      <span style={{ fontSize: "1rem", color: borderColors[index % borderColors.length], fontWeight: 700 }}>0{index + 1}</span>
      <span style={{ fontSize: isMobile ? "0.85rem" : "0.95rem", fontWeight: 700, color: "#111827", lineHeight: 1.4 }}>
    ...
    <div>
      ...
      <section style={{ padding: isMobile ? "3rem 1.5rem 4rem" : "6rem 5rem 8rem", background: "#fff" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontSize: "1.8rem", fontWeight: 700, color: "#111827", margin: "0 0 1.2rem" }}>품질경영</h2>
          <p style={{ fontSize: "0.95rem", color: "#6b7280", marginBottom: "0.5rem" }}>품질은 고객과의 약속이며 기업의 경쟁력입니다.</p>
          ...
          <p style={{ fontSize: "0.95rem", color: "#6b7280", lineHeight: 1.8, marginBottom: "7rem" }}>
    ...
    <div style={{ display: "flex", alignItems: "stretch", overflowX: "auto" }}>
      {processSteps.map((step, i) => {
        ...
        return (
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: "1.5rem", marginBottom: "7rem" }}>
            {[
          ...
          <div>
            ...
            {/* 한 줄 배치 */}
            <div style={{ display: "flex", justifyContent: "center", gap: isMobile ? "1rem" : "2rem", flexWrap: "wrap", marginBottom: "7rem" }}>
              {goals.map((item, i) => (
                <GoalCircle key={item} index={i} label={item} isMobile={isMobile} />
              ))}
            </div>
