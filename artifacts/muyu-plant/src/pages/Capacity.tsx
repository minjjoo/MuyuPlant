...
const products = [
  { name: "Pressure Vessel", nameKo: "압력용기", desc: "고압 환경에서 기체나 액체를 안전하게 저장·처리하는 용기. 석유화학, 발전, 가스 산업 전반에 공급합니다." },
  { name: "Heat Exchanger", nameKo: "열교환기", desc: "서로 다른 온도의 유체 사이에서 열을 교환하는 설비. Shell & Tube 방식을 주력으로 제작합니다." },
  { name: "Tower & Column", nameKo: "탑류 설비", desc: "증류, 흡수, 세정 등의 공정에 사용되는 탑형 설비. 대형 플랜트 현장 납품 실적 다수 보유." },
  { name: "Storage Tank", nameKo: "저장 탱크", desc: "액체·가스 원료의 저장 및 이송을 위한 탱크류. 다양한 규격 및 소재 맞춤 제작 가능." },
];
    ...
    <div>
      ...
      <section style={{ padding: "8rem 5rem", background: "#fff" }}>
        <div style={{ maxWidth: "960px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "0", borderTop: "1px solid #e5e7eb", borderLeft: "1px solid #e5e7eb" }}>
            {products.map(({ name, nameKo, desc }) => (
              <div key={name} style={{ padding: "2rem 1.8rem", borderRight: "1px solid #e5e7eb", borderBottom: "1px solid #e5e7eb" }}>
                <div style={{ fontWeight: 700, fontSize: "0.95rem", color: "#111827", marginBottom: "0.3rem" }}>{nameKo}</div>
                <div style={{ fontSize: "0.77rem", color: "#9ca3af", marginBottom: "0.9rem" }}>{name}</div>
